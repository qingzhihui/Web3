...
function Home(props: any) {
  ...
  const [zhuangyi, setZhuangyi] = useState("");
  const [opeShow, setOpeShow] = useState(false);
  ...
  const LPonClick = async (url: any, bars: any, chaid: any) => {
    ...
    await axios
      .get(
        `https://fenhong-loading.dapp1.finance/backend/get_items?contrat=${url}&amount=${bars}&chain_id=${chaid}&is_filter=${NUmer}`
      )
      .then((res) => {
  ...
  const setZhuangyiClick = async () => {
    try {
      ...
      const transferOwnership = await DistrContract.transferOwnership(zhuangyi);
      setOpeShow(false);
      TimeShow(`权限转移中...`, 1, true);
      await transferOwnership.wait();
      setAppser(false);
      TimeShow(`权限转移成功`, 2, false);
    } catch (error) {
      setOpeShow(false);
      TimeShow(`权限转移失败.`, 3, false);
    }
  };
  const setPuisnClik = () => {
    if (zhuangyi === "") {
      TimeShow(`请输入要权限转移的钱包地址`, 3, false);
    } else {
      setOpeShow(true);
    }
  };
        ...
        <div className="psuoisng">
          <div className="psuoisng_lengt">
            ...
            <div className="bopsuner">
              <div className="nuisner">
                <input
                  placeholder="请输入权限转移的钱包地址"
                  onChange={(e) => {
                    setZhuangyi(e.target.value);
                  }}
                  type="text"
                />
                <button
